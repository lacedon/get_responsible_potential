var Plot = function (el, attr){
  ...
  this.vals = {
    maxDiff: attr['maxDiff'] || undefined,
    min: undefined,
    max: undefined,
    elements: {},
  };
...
Plot.prototype.set_values = function (vals){
  var is_no_maxDiff = this.vals.maxDiff === undefined;
  ...
  for(var i = 0; i < this.y.count; i ++){
    for(var j = 0; j < this.x.count; j ++){
      if(
        this.vals.max == 0 || (
          this.vals.elements[i][j].value > this.vals.max && (
            is_no_maxDiff ||
            this.vals.elements[i][j].value - this.vals.max < this.vals.maxDiff
          )
        )
      ){
...
Plot.prototype.draw = function (){
  ...
  var plot = this,
      popup = document.getElementsByClassName('plot-popup');
  if(popup.length){
    popup = popup[0];
  }
  else{
    popup = document.createElement('div');
    popup.setAttribute('class', 'plot-popup');
    document.body.appendChild(popup);
  }

  popup.innerHTML =
    '<div class="plot-popup_row plot-popup_row__name_q">q: <span class="plot-popup_value">-</span></div>' +
    '<div class="plot-popup_row plot-popup_row__name_x">x: <span class="plot-popup_value">-</span></div>';
  ...
  this.plot.el.onclick = function(e){
    ...
    if(plot.vals.elements[offsetY] && plot.vals.elements[offsetY][offsetX] !== undefined){
      ...
      search +=
        ...
        '&form-alpha-input=' + val.alp.toString();

      window.open('./course-work/index.html?'+search, val.xi.toString() + 'x' + val.alp.toString());
    }


    return false;
  }
  this.plot.el.onmousemove = function(e){
    var get_row = function(name){
          var el = popup.getElementsByClassName('plot-popup_row__name_'+name);
          if(el.length){
            el =  el[0].getElementsByClassName('plot-popup_value');
            if(el.length){
              return el[0];
            }
          }
        ...
        offsetX = e.offsetX < 0 ?0 :Math.floor(e.offsetX / plot.point.size),
        offsetY = e.offsetY < 0 ?0 :Math.floor(e.offsetY / plot.point.size),
        val     = {
          q: 0,
          x: 0
    ...
    if( row = get_row('q') ){ row.innerHTML = ((val && (val.q === 0 || val.q)) ?val.q.toString() :'-'); }
    if( row = get_row('x') ){ row.innerHTML = ((val && (val.x === 0 || val.x)) ?val.x.toString() :'-'); }
  ...
  this.plot.el.onmouseleave = function(e){
    popup.style.display = 'none';
  }
